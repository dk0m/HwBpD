...
VOID BLOCK_REAL(PCONTEXT ctx) {
    ctx.Rip = cast(ULONG_PTR)&ucRet;
}

bool initHwBpVars() {
    
    RtlZeroMemory(&g_CritSection, CRITICAL_SECTION.sizeof);
    RtlZeroMemory(&g_DetourFuncs[0], g_DetourFuncs.sizeof);
    ...
    if (!g_CritSection.DebugInfo) {
        InitializeCriticalSection(&g_CritSection);
    }

    return g_Veh && g_CritSection.DebugInfo;
